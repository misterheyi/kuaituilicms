...
$(function() {
	...
	//初始化数据
	$('#dg').datagrid({
		url : basePath + 'riddle/selectRiddleList',
		loadMsg : '数据加载中请稍后……',
		selectOnCheck : true,
		checkOnSelect : true,
		method : 'post',
	...
	//
	$("#searchRiddleBtn").click(function() {
		var params = $('#dg').datagrid('options').queryParams; // 先取得 //
		// // 的查询参数
		params.query = $("#riddleSearchName").val();
	...
	function addrow() {
		$('#win').window({
  			title:'添加密语',
  			href:basePath + 'riddle/popRiddleWindow',
  			width:1000,
  			height:800,
...
function addOrUpdateRiddle(dom) {
	$.ajax({
		url : basePath + '/riddle/addOrUpdate',
		data : {
			"id" : $("#id").val(),
