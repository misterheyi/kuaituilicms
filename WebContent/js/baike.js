...
$(function() {
	...
	//初始化数据
	$('#dg').datagrid({
		url : basePath + 'baike/selectBaikeList',
		loadMsg : '数据加载中请稍后……',
		selectOnCheck : true,
		checkOnSelect : true,
		method : 'post',
	...
	//
	$("#searchBaikeBtn").click(function() {
		var params = $('#dg').datagrid('options').queryParams; // 先取得 //
		// // 的查询参数
		params.query = $("#baikeSearchName").val();
	...
	function addrow() {
		$('#win').window({
  			title:'添加短文',
  			href:basePath + 'baike/popBaikeWindow',
  			width:1000,
  			height:800,
...
function addOrUpdateBaike(dom) {
	...
	$.ajax({
		url : basePath + '/baike/addOrUpdate',
		data : {
			"id" : $("#id").val(),
