...
$(function() {
	...
	//初始化数据
	$('#dg').datagrid({
		url : basePath + 'essay/selectEssayList',
		loadMsg : '数据加载中请稍后……',
		selectOnCheck : true,
		checkOnSelect : true,
		method : 'post',
	...
	//
	$("#searchEssayBtn").click(function() {
		var params = $('#dg').datagrid('options').queryParams; // 先取得 //
		// // 的查询参数
		params.query = $("#essaySearchName").val();
	...
	function addrow() {
		$('#win').window({
  			title:'添加短文',
  			href:basePath + 'essay/popEssayWindow',
  			width:1000,
  			height:800,
	    ...
	    $('#win').window({
	    	title:'更新短文',
  			...
  			onLoad: function(){
  				...
  				$('#type').combobox("setValue",item.type);//这里写设置默认
...
function addOrUpdateEssay(dom) {
	...
	$.ajax({
		url : basePath + '/essay/addOrUpdate',
		data : {
			"id" : $("#id").val(),
