...
$(function() {
	...
	//初始化数据
	$('#dg').datagrid({
		url : basePath + 'mystery/selectMysteryList',
		loadMsg : '数据加载中请稍后……',
		selectOnCheck : true,
		checkOnSelect : true,
		method : 'post',
	...
	//
	$("#searchMysteryBtn").click(function() {
		var params = $('#dg').datagrid('options').queryParams; // 先取得 //
		// // 的查询参数
		params.query = $("#mysterySearchName").val();
	...
	function addrow() {
		$('#win').window({
  			title:'添加短文',
  			href:basePath + 'mystery/popMysteryWindow',
  			width:1000,
  			height:800,
...
function addOrUpdateMystery(dom) {
	...
	$.ajax({
		url : basePath + '/mystery/addOrUpdate',
		data : {
			"id" : $("#id").val(),
